    ...
    try {
        ...
        for (let i = 0; i < csvData.length; i++) {
            try {
                ...
                // Map CSV columns to Creator schema
                const creatorData = {
                    name: row.name || row.Name || '',
                    genre: row.genre || row.Genre || 'Other',
                    avatar: row.avatar || row.Avatar || 'https://via.placeholder.com/400x400?text=Avatar',
                    platform: row.platform || row.Platform || 'Instagram',
                    socialLink: row.socialLink || row.SocialLink || row.social_link || '',
                    location: row.location || row.Location || 'Other',
                    phoneNumber: row.phoneNumber || row.PhoneNumber || row.phone_number || '',
                    mediaKit: row.mediaKit || row.MediaKit || row.media_kit || '',
                    details: {
                        bio: row.bio || row.Bio || row.description || row.Description || '',
                        location: row.location || row.Location || 'Other',
                        analytics: {
                            followers: parseInt(row.followers || row.Followers || 0),
                            totalViews: parseInt(row.totalViews || row.TotalViews || row.total_views || 0),
                            averageViews: parseInt(row.averageViews || row.AverageViews || row.average_views || 0)
                        },
                        reels: []
                    }
                };
